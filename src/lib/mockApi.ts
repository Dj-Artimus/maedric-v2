...
export class MockApiService {
  ...
  private generateProductsWithFilters() {
    ...
    console.log(
      `Generated ${baseProducts.length} total products (95 base + ${additionalProductCount} guaranteed)`
    );
  ...
  async getProducts(
    ...
    if (filters.metalTypes && filters.metalTypes.length > 0) {
      filteredProducts = filteredProducts.filter((product) =>
        filters.metalTypes!.includes(product.metalType)
      );
    }
    ...
    if (
      filters.ringSize &&
      filters.ringSize.size &&
      filters.ringSize.size !== "6"
    ) {
      filteredProducts = filteredProducts.filter(
    ...
    if (filters.gemstoneUsage && filters.gemstoneUsage.length > 0) {
      filteredProducts = filteredProducts.filter((product) =>
        filters.gemstoneUsage!.includes(product.gemstoneUsage)
      );
    }
